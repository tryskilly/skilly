...
| `src/db/*` | `WebBackendRepo` interface + Postgres (`pg`) + in-memory (seeded demo) impls; `getRepo()` picks by `DATABASE_URL`. Full dashboard + billing + admin ops: key CRUD, skill save, usage summary + `listUsageEvents`, `setTenantUsageCap` + `setTenantPolarCustomerId`, tenant create/rename, membership upsert/list/delete, widget config get/save. |
| `src/db/schema.ts` + `db/migrations/*` | Drizzle ORM schema (tenants incl. `polar_customer_id`, api_keys, tenant_skills, usage_events, **dashboard_memberships**, **tenant_widget_configs**) + generated migrations (`bun run db:generate` / `bun run db:migrate`). |
| `src/tenantService.ts` | Framework-free auth → quota → mint orchestration. |
| `src/app/api/web/{token,skill,usage,checkout,portal}/route.ts` + `webhooks/polar` | Routes: mint token, serve SKILL.md, meter session seconds, start checkout, **customer-portal session**, Polar webhook (verified → set cap + persist customer id). |
| `src/app/api/auth/workos/{start,callback}` + `api/dashboard/{login,logout,setup-db}` | WorkOS AuthKit sign-in + signed state, emergency password fallback, DB bootstrap (super_admin + `SKILLY_DB_SETUP_ENABLED`). |
| `src/app/dashboard/**` | Dashboard UI (Tailwind): overview with conditional setup badge, install, **widget config** (accent/locale/launcher), origins, keys, SKILL.md editor (validate-on-save), usage (monthly meter + recent-events table), billing, settings, and **super-admin** tenant directory + drill-in member management + sidebar tenant switcher. Server `actions.ts` covers every mutation. |
| `src/lib/{dashboardAuth,workosAuth,session,readiness,requestOrigin}.ts` | Signed HTTP-only session cookie, WorkOS AuthKit + OAuth state, tenant resolution, readiness probe, request-origin helpers. |
| `src/lib/{analytics}.ts` | PostHog + GA server-side event capture (no transcript text). |

> Env: `OPENAI_API_KEY`, `POSTGRES_URL`/`DATABASE_URL` (optional), `POLAR_{ACCESS_TOKEN,PRODUCT_ID,WEBHOOK_SECRET,PLAN_CAP_SECONDS}`, WorkOS `WORKOS_{CLIENT_ID,API_KEY,DASHBOARD_REDIRECT_URI}`, `SKILLY_DASHBOARD_{PASSWORD,SESSION_SECRET,ROLE}`. Validated: `bun test` **59/59**, `tsc` + `next build` clean, and Playwright end-to-end — token (401/403/500/200), dashboard (key reveal, skill validation, app-id add/remove), **billing** (meter → signed webhook sets cap + customer id → portal → dashboard shows new plan), and **mobile app-id tenancy** (9.0): a native request (`X-Skilly-App-Id`, no Origin) authenticates against the app-id allowlist; a web request with a disallowed Origin can't bypass it via a spoofed app-id (**403**). Internal imports are extensionless (Next webpack doesn't resolve `.js`→`.ts`). The web SDK is functionally complete (8.0–8.6); the backend now also serves the mobile SDK (Phase 9.0).
