...
| `OnboardingPermission.swift` | ~45 | Pure ordered permission model for the guided first-run flow. Selects the next required permission and calculates progress independently of SwiftUI. |
...
| `sdk/web/src/widget.ts` | Shadow-DOM visitor UI: premium amber launcher, fixed/draggable conversation panel with bounded history, independent pointer-following speech caption, consent/connection/listening/speaking/pointing/error/quota/mic-denied states, typed-input fallback, guided-step list, action confirmation, and cursor positioning. |
