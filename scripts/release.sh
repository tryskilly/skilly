...
# Keep the release artifact name aligned with PRODUCT_NAME in Xcode.
APP_NAME="${APP_NAME:-Skilly}"
...
DMG_SOURCE_DIR="${BUILD_DIR}/dmg-source"
CREATE_DMG_LOG="${BUILD_DIR}/create-dmg.log"

echo "💿 Creating DMG..."
if [ ! -d "${EXPORT_DIR}/${APP_NAME}.app" ]; then
    echo "❌ Exported app not found at ${EXPORT_DIR}/${APP_NAME}.app"
    exit 1
fi

# Stage only the .app in a clean source folder so the DMG does not include
# export logs/plists generated by xcodebuild.
rm -rf "${DMG_SOURCE_DIR}"
mkdir -p "${DMG_SOURCE_DIR}"
ditto "${EXPORT_DIR}/${APP_NAME}.app" "${DMG_SOURCE_DIR}/${APP_NAME}.app"
...
create_dmg_once() {
    ...
    if [ "${sandbox_mode}" = "sandbox-safe" ]; then
        create_dmg_args=(--sandbox-safe "${create_dmg_args[@]}")
    fi
...
: > "${CREATE_DMG_LOG}"
create_dmg_mode="standard"
set +e
create_dmg_once "${create_dmg_mode}"
create_dmg_status=$?
set -e

if [ "${create_dmg_status}" -ne 0 ] && grep -Eq "Operation not permitted|could not access /Volumes/" "${CREATE_DMG_LOG}"; then
    echo "   create-dmg hit a /Volumes permission error; retrying with --sandbox-safe..."
    create_dmg_mode="sandbox-safe"
    set +e
    create_dmg_once "${create_dmg_mode}"
    create_dmg_status=$?
    set -e
...
if [ "${create_dmg_status}" -ne 0 ]; then
    echo "❌ DMG creation failed. Last 40 log lines:"
    tail -40 "${CREATE_DMG_LOG}" || true
    exit 1
fi

tail -5 "${CREATE_DMG_LOG}" || true
if [ "${create_dmg_mode}" = "sandbox-safe" ]; then
    echo "   DMG was created in sandbox-safe mode (mounting under /private/tmp instead of /Volumes)."
fi
...
gh release create "${TAG}" "${DMG_PATH}" \
    ...
    --notes "${APP_NAME} v${MARKETING_VERSION}" \
    --latest

# ── Step 9: Push appcast.xml to the releases repo ───────────────────────────
# Use GitHub's contents API via `gh` to avoid interactive git clone/push auth
# stalls in automated terminal runs.
...
APPCAST_API_PATH="repos/${GITHUB_REPO}/contents/appcast.xml"
APPCAST_SHA=$(gh api "${APPCAST_API_PATH}" --jq '.sha' 2>/dev/null || echo "")
APPCAST_BASE64=$(base64 < "${PROJECT_DIR}/appcast.xml" | tr -d '\n')
APPCAST_PAYLOAD=$(mktemp)

if [ -n "${APPCAST_SHA}" ]; then
    cat > "${APPCAST_PAYLOAD}" <<JSON
{"message":"Update appcast.xml for v${MARKETING_VERSION}","content":"${APPCAST_BASE64}","branch":"${DEFAULT_BRANCH}","sha":"${APPCAST_SHA}"}
JSON
else
    cat > "${APPCAST_PAYLOAD}" <<JSON
{"message":"Add appcast.xml for v${MARKETING_VERSION}","content":"${APPCAST_BASE64}","branch":"${DEFAULT_BRANCH}"}
JSON
fi

gh api --method PUT "${APPCAST_API_PATH}" --input "${APPCAST_PAYLOAD}" >/dev/null
rm -f "${APPCAST_PAYLOAD}"
echo "✅ appcast.xml pushed to ${DEFAULT_BRANCH}"
