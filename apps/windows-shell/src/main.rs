//! Windows shell bootstrap smoke runner.
//!
...
fn acquire_auth_session() -> AuthSession {
    AuthSession {
        workos_user_id: "windows-smoke-user".to_string(),
...
fn run_smoke_flow() -> Result<ShellSmokeResult, String> {
    ...
    let events = vec![
        RealtimeEvent::TurnStarted {
            turn_id: "windows-smoke-turn-1".to_string(),
        },
...
fn main() {
    ...
    if !smoke_requested {
        println!("skilly-windows-shell bootstrap ready (use --smoke)");
        return;
    }
    ...
    match run_smoke_flow() {
        Ok(smoke_result) => {
            println!(
                "windows shell smoke passed: user={} allowed={} phase={} turns_completed={}",
        ...
        Err(error_message) => {
            eprintln!("windows shell smoke failed: {error_message}");
            std::process::exit(1);
        }
    }
}
