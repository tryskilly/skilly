...
export class PostgresRepo implements WebBackendRepo {
  ...
  async applyTenantBillingEvent(input: { eventId: string; tenantId: string; capSeconds: number; polarCustomerId?: string | null; providerEventAt?: string | null; providerState?: string | null }): Promise<{ replay: boolean; applied: boolean }> {
    ...
    try {
      ...
      const updated = await client.query(`UPDATE tenants SET usage_cap_seconds=$2, polar_customer_id=COALESCE($3, polar_customer_id), provider_event_at=$4, provider_event_id=$1, provider_state=$5 WHERE id=$6 AND (provider_event_at IS NULL OR ($4 IS NOT NULL AND ($4 > provider_event_at OR ($4 = provider_event_at AND CASE WHEN $5 IN ('revoked','past_due') THEN 2 WHEN $5='canceled' THEN 1 ELSE 0 END > CASE WHEN provider_state IN ('revoked','past_due') THEN 2 WHEN provider_state='canceled' THEN 1 ELSE 0 END))))`, [input.eventId, input.capSeconds, input.polarCustomerId ?? null, input.providerEventAt ?? null, input.providerState ?? null, input.tenantId]);
      await client.query("COMMIT");
