...
export class PostgresRepo implements WebBackendRepo {
  ...
  async applyTenantBillingEvent(input: { eventId: string; tenantId: string; capSeconds: number; polarCustomerId?: string | null; providerEventAt?: string | null; providerState?: string | null }): Promise<{ replay: boolean; applied: boolean }> {
    ...
    try {
      await client.query("BEGIN");
      const claimed = await client.query(`INSERT INTO polar_webhook_events (event_id, provider_event_at) VALUES ($1, $2) ON CONFLICT (event_id) DO NOTHING`, [input.eventId, input.providerEventAt ?? null]);
      if (claimed.rowCount !== 1) { await client.query("ROLLBACK"); return { replay: true, applied: false }; }
      const updated = await client.query(`UPDATE tenants SET usage_cap_seconds=$2, polar_customer_id=COALESCE($3, polar_customer_id), provider_event_at=$4::timestamptz, provider_event_id=$1, provider_state=$5 WHERE id=$6 AND (provider_event_at IS NULL OR ($4::timestamptz IS NOT NULL AND ($4::timestamptz > provider_event_at OR ($4::timestamptz = provider_event_at AND CASE WHEN $5 IN ('revoked','past_due') THEN 2 WHEN $5='canceled' THEN 1 ELSE 0 END > CASE WHEN provider_state IN ('revoked','past_due') THEN 2 WHEN provider_state='canceled' THEN 1 ELSE 0 END))))`, [input.eventId, input.capSeconds, input.polarCustomerId ?? null, input.providerEventAt ?? null, input.providerState ?? null, input.tenantId]);
      await client.query("COMMIT");
      return { replay: false, applied: updated.rowCount === 1 };
